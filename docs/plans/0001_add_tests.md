...
- [x] 3. Test `Commands.Apps.App`: add `AppCommand` behaviour (with `@callback` before each relevant `def`); use `Application.compile_env` in `Commands.Apps.App`; declare `DokkuRemote.AppCommand.Mock` in `test/support/mocks.ex`; add config entry; write tests.
- [x] 4. Test `Commands.Config.App`: use `Application.compile_env` and `DokkuRemote.AppCommand.Mock`; write tests.
